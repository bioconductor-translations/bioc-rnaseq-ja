...
ポリA末端を持つ分子（主にmRNA）をエンリッチメントする、あるいは、そうでなければ非常に豊富なリボソームRNAを除去した後、残りの分子を細かく断片化する（分子全体を考慮するロングリード・プロトコルもあるが、このレッスンの焦点ではない）。
...
リードをゲノムにアライメントし、既知のアノテーション付きフィーチャーとのオーバーラップを調べる場合、全ゲノム配列（fastaファイルで提供）と、各アノテーション付きフィーチャーのゲノム上の位置を示すファイル（通常gtfファイルで提供）が必要です。
...
また、上位にランクされた遺伝子間に機能的関係があるかどうかを調べるために、遺伝子セット（エンリッチメント）解析と呼ばれるフォローアップ解析を行うことも一般的であるが、これについても後のエピソードで取り上げる。
...
- アノテーション付き遺伝子に関する情報は、Ensembl、UCSC、GENCODEなどいくつかの情報源からアクセスできる。
