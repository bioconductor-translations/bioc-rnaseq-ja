...
`DESeq2`パッケージの`nbinomWaldTest()`関数を使用することで、_一般化線形モデル（GLM）_を構築し、 _design matrices_ の各変数に対応する_対数2倍変化量_（「GLM係数」「ベータ係数」または「効果量」と同義）を算出できます。 _design matrices_ は _design formula_ と直接関連しており、自動的に生成されます。例えば、1つの変数（`~ treatment`：処理群と対照群）を持つdesign formulaで、2つの因子水準（処理群と対照群）がある場合を考えます。この場合、サンプル$j$における特定遺伝子の平均発現量$\mu_{j}$は以下のようにモデル化されます：
